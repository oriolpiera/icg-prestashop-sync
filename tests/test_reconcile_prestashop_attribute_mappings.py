...
class TestReconcilePrestashopAttributeMappingsCommand:
    ...
    def test_apply_prefers_prestashop_product_id_color_group_over_reference_group(self):
        product = _make_product("2830001")
        product.prestashop_id = 2945
        ...
        color_group.refresh_from_db()
        assert color_group.prestashop_id == 77
        assert "groups=1 values=0" in out.getvalue()

    def test_apply_prefers_product_specific_size_group_over_generic_size(self):
        product = _make_product("2830001")
        product.prestashop_id = 2945
        product.save(update_fields=["prestashop_id"])
        size_group = AttributeGroup.objects.create(
