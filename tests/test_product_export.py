...
class TestPrestashopClientProductExport:
    ...
    def test_preserves_existing_categories_on_product_update(self, settings):
        product = _make_product(name="Updated Product")
        session = Mock()
        session.request.side_effect = [
            _response(
                ...
                "<visibility>both</visibility>"
                "<minimal_quantity>1</minimal_quantity>"
                "<id_category_default>500</id_category_default>"
                "<name><language id='1'>Old name</language></name>"
                "<link_rewrite><language id='1'>old-name</language></link_rewrite>"
                "<associations>"
                "<categories>"
                "<category><id>500</id></category>"
                "<category><id>600</id></category>"
                "</categories>"
                "</associations>"
                "</product></prestashop>"
            ),
        ...
        put_call = session.request.call_args_list[1]
        payload = put_call.kwargs["data"]
        assert "<id>500</id>" in payload
        assert "<id>600</id>" in payload
        assert "<id>251</id>" not in payload
        assert "<id_category_default>500</id_category_default>" in payload

    def test_sets_categories_on_product_create(self, settings):
        product = _make_product(name="New Product")
        session = Mock()
        session.request.side_effect = [
            _response(
                "<prestashop><product>"
