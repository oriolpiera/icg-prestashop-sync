...
@pytest.mark.django_db
def test_refresh_order_from_prestashop_preserves_override_when_mutable_line_fields_change():
    override_product = Product.objects.create(
        icg_id=5002,
        prestashop_id=102,
        reference="MUG-002",
    ...
    line = refreshed.lines.get(position=1)
    assert line.override_combination_id == override_combination.pk
    assert line.description == "Blue mug updated"
...
def test_refresh_order_from_prestashop_preserves_override_when_line_position_changes():
    ...
    assert moved_line.prestashop_product_id == 101
    assert moved_line.prestashop_combination_id == 202
